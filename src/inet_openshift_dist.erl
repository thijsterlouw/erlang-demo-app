...
listen(Name) ->
	...
	{ok, Socket} ->
	    ...
	    ExternalPort = get_external_port(Port),
	    info_msg("LISTEN PORT ~p , REGISTERERING ~p IN EPMD\n", [Port, ExternalPort]),
	    {ok, Creation} = erl_epmd:register_node(Name, ExternalPort),
	    {ok, {Socket, TcpAddress, Creation}};
	Error ->
	    Error
    end.

get_external_port(Port) ->
	case application:get_env(kernel,inet_dist_listen_external) of
